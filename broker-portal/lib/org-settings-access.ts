 ...
 * be the unbuilt ones — since BACKLOG-3098 nothing in the portal does. It asks
 * the database, per key, and renders the answer.
 *
 ...
 * card would vanish instead of graying. Kept apart, a broken plan RPC still
...
export async function resolveOrgSettingsFeatures(
  ...
  return {
    retention: toCardPolicy(RETENTION_FEATURE_KEY, retention, built),
    scim: toCardPolicy(SCIM_FEATURE_KEY, scim, built),
    jit: toCardPolicy(JIT_FEATURE_KEY, jit, built),
  };
}
 ...
 *
 * These literals are NOT an exception to BACKLOG-3098's "the database decides".
 * A support session cannot read feature_definitions either (no authenticated
 * user, and the read policy is scoped to `authenticated`), so the normal path
 * would return an empty built-set and hide these same two cards. This function
 * states that outcome directly rather than making a doomed round trip.
 */
