...
import {
  ...
  safeAuthErrorInfo,
} from '@/lib/supabase/cookie-guard';
...
describe('isBareAuthTokenCookie', () => {
  ...
  it('flags a bare-JSON chunk', () => {
    expect(
      isBareAuthTokenCookie('sb-nercleijfrxqcvfjskbc-auth-token.0', BARE_SESSION_JSON)
    ).toBe(true);
  });
...
describe('safeAuthErrorInfo', () => {
  it('extracts name/code and NEVER the session-bearing message', () => {
    ...
    const err = new TypeError(
      `Cannot create property 'user' on string '${BARE_SESSION_JSON}'`
    );
