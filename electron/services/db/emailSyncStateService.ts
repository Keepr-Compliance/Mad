...
export function resolveMailboxAccountId(
  ...
  const row = dbGet<{ id: string }>(
    unsafeSql("SELECT id FROM oauth_tokens WHERE user_id = ? AND provider = ? AND purpose = 'mailbox' LIMIT 1"),
    [userId, provider],
  );
  return row?.id ?? null;
}
...
export function getSyncState(userId: string, accountId: string): EmailSyncStateRow | undefined {
  return dbGet<EmailSyncStateRow>(
    unsafeSql("SELECT * FROM email_sync_state WHERE user_id = ? AND account_id = ?"),
    [userId, accountId],
  );
}
...
export function ensureSyncStateRow(
  ...
  dbRun(
    unsafeSql(`INSERT OR IGNORE INTO email_sync_state (user_id, account_id, provider)
     VALUES (?, ?, ?)`),
...
export function updateCachedBounds(
  ...
  if (existing) {
    dbRun(
      unsafeSql(`UPDATE email_sync_state
    ...
    dbRun(
      unsafeSql(`INSERT INTO email_sync_state (user_id, account_id, provider, newest_cached_at, oldest_cached_at)
       VALUES (?, ?, ?, ?, ?)`),
