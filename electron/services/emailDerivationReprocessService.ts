 ...
 * duration, and a migration cannot `await`. v66 therefore adds the column and the
...
export async function reprocessEmailDerivations(
  ...
  // if v66 has not run. Either way there is nothing to do — and neither is an
...
/** True when `emails` exists AND carries `derived_version` (i.e. v66 has run). */
