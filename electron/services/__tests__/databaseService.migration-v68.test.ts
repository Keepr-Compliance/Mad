 ...
 *
 * Integration test for migration v68 (BACKLOG-2859 — the role collapse).
 *
 * RENUMBERED 66 -> 68 when this landed on int/ui-polish-e, where 66 is
 * BACKLOG-2814 (message_thread_names) and 67 is BACKLOG-2857
 * (emails.derived_version). On 2859's own branch (PR #2381) the migration is
 * still 66 and correct there — its base is feat/BACKLOG-2849-submit-screen,
 * whose chain head is 65, identical to develop. Nothing but the number moved.
 *
 * v68 does two things, both pure DML:
 ...
 *     throw.
 *
 * Follows the v47..v67 convention: real better-sqlite3 driver, in-memory DB via
 * createMigrationHarness, seeded at 67 AND clipped to 68 so ONLY v68 runs.
 *
 * Clipping with `filter(m => m.version <= 68)` keeps a CONTIGUOUS PREFIX, which
...
const USER_ID = "user-v68-test";
const TXN_LISTING = "txn-v68-listing"; // stored `purchase`
const TXN_SALE = "txn-v68-sale";

/** Post-v67 / pre-v68 shape — only what v68 reads. */
const PRE_V68_FIXTURE = `
...
/**
 * The seed. Every branch v68 can take has at least one row, and the untouched
...
describe("databaseService migration v68 (BACKLOG-2859 — role collapse)", () => {
  let harness: MigrationHarness;

  beforeEach(() => {
    harness = createMigrationHarness({ seedV29Schema: false });
    harness.db.exec(PRE_V68_FIXTURE);
  ...
  /** Seed at v67 AND clip the chain at v68 so ONLY v68 runs. */
  async function runV68(): Promise<void> {
    harness.db.prepare("INSERT OR REPLACE INTO schema_version (id, version) VALUES (1, 67)").run();
    const klass = harness.service.constructor as { MIGRATIONS: Array<{ version: number }> };
    const all = klass.MIGRATIONS;
    klass.MIGRATIONS = all.filter((m) => m.version <= 68);
    try {
  ...
  it("advances schema_version to 68", async () => {
    await runV68();
    expect(schemaVersion(harness.db)).toBe(68);
  });
  ...
  it("no-ops when the tables are absent", async () => {
    ...
    await expect(runV68()).resolves.toBeUndefined();
    expect(schemaVersion(harness.db)).toBe(68);
  });
});
