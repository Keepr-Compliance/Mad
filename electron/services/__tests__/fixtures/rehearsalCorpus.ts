 ...
 *
 * FAY / GUS / HAL / IVY EXIST TO MAKE MIGRATION v66 NON-VACUOUS. Before they
 ...
 * so four of v66's six operations matched ZERO ROWS and the rehearsal passed
 ...
 *
 * Distinct from PARTIES_BY_TRANSACTION below because migration v66 removes two
...
export const PARTIES_BY_TRANSACTION: Readonly<Record<string, readonly string[]>> = {
  // gus and hal are ABSENT BY DESIGN, not by omission: migration v66 deletes
  // their assignments (see TRANSACTION_CONTACTS_REMOVED_BY_V66). This set is
...
/**
 * The transaction_contacts rows migration v66 DELETES — the counterparty
 ...
 */
export const TRANSACTION_CONTACTS_REMOVED_BY_V66: readonly string[] = [
  "tc-2700-open-gus",
  "tc-2700-frozen-hal",
];
...
/** The rows that SURVIVE the chain: everything seeded, minus the v66 deletions. */
export const TRANSACTION_CONTACT_IDS: readonly string[] =
  TRANSACTION_CONTACT_IDS_SEEDED.filter(
    (id) => !TRANSACTION_CONTACTS_REMOVED_BY_V66.includes(id),
  );

/**
 * The exact role each surviving row holds AFTER the chain — the structural
 * probe for migration v66, in the style of the existing `v56 applied:` /
 * `v62 applied:` tests.
 *
 * Asserted by id so it cannot be satisfied by the right number of rows carrying
 * the wrong roles. Drop v66 from the chain and all four collapsed rows go red
 * at once.
 */
export const TRANSACTION_CONTACT_ROLES_AFTER_V66: Readonly<Record<string, string>> = {
...
export const CONTACT_DEFAULT_ROLES_AFTER_V66: Readonly<Record<string, string | null>> = {
...
export const EXPECTED_ROW_COUNTS: Readonly<Record<string, number>> = {
  ...
  contact_phones: 3,
  transactions: 2,
  // 9 seeded minus the 2 counterparty-principal assignments v66 removes.
