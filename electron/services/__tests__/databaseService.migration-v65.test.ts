 ...
 *
 * Integration test for migration v65 (BACKLOG-2791 — the persistent Needs-Review
 ...
 *
 * Follows the v47..v64 convention: real better-sqlite3 driver, in-memory DB via
 * createMigrationHarness, seeded at 64 AND clipped to 65 so ONLY v65 runs.
 */
...
const USER_ID = "user-v65-test";
const TXN_A = "txn-v65-a";
const TXN_B = "txn-v65-b";
const EMAIL_ID = "email-v65-alpha";

/** Post-v64 / pre-v65 shape. */
const PRE_V65_FIXTURE = `
...
/** The column set `transactions` carries BEFORE v65, in order. */
const PRE_V65_TXN_COLUMNS = [
  "id",
  "user_id",
  "property_address",
  "status",
...
describe("databaseService migration v65 (BACKLOG-2791 — pending review queue)", () => {
  let harness: MigrationHarness;

  beforeEach(() => {
    harness = createMigrationHarness({ seedV29Schema: false });
    harness.db.exec(PRE_V65_FIXTURE);
  ...
  /** Seed at v64 AND clip the chain at v65 so ONLY v65 runs. */
  async function runV65(): Promise<void> {
    harness.db.prepare("INSERT OR REPLACE INTO schema_version (id, version) VALUES (1, 64)").run();
    const klass = harness.service.constructor as { MIGRATIONS: Array<{ version: number }> };
    const all = klass.MIGRATIONS;
    klass.MIGRATIONS = all.filter((m) => m.version <= 65);
    try {
  ...
  it("advances schema_version to 65", async () => {
    await runV65();
    expect(schemaVersion(harness.db)).toBe(65);
  });
  ...
  it("is idempotent — re-running changes nothing and does not throw", async () => {
    ...
    harness.db.prepare("INSERT OR REPLACE INTO schema_version (id, version) VALUES (1, 64)").run();
    await expect(runV65()).resolves.not.toThrow();
