 ...
 *
 * Integration test for migration v67 (BACKLOG-2857 — derivation provenance).
 *
 * RENUMBERED 66 -> 67 ON int/ui-polish-e, exactly as v65 was renumbered from 64
 * after PR #2346. This item was written against develop, where the chain head is
 * 65, so 66 was correct on its own branch; PR #2368 (BACKLOG-2814, Apple
 * group-chat names) reached the integration branch first and holds 66. The two
 * are disjoint — 66 creates `message_thread_names`, 67 adds a column to `emails`
 * — so execution order between them does not matter.
 *
 * The renumber exists ONLY here. 2857's own branch keeps 66, because
 * validateNoVersionGaps throws `Missing migration version 66` on EVERY database
 * init when the chain skips a number, so a branch holding 67 without 66 present
 * would not boot and could not run this suite at all.
 *
 * THE FIXTURE BELOW IS NOT MISSING `message_thread_names`. It is seeded at 66,
 * so v66 never runs, and v66 does not touch `emails` — the table under test here
 * is byte-identical either way. Do not "complete" the fixture from schema.sql;
 * see the vacuity trap note.
 *
 * v67 adds:
 ...
 * `schema.sql` already has the column, and an assertion of the form "after v67 the
 * column exists" passes WITHOUT v67 doing anything at all. That is precisely the
 ...
 *
 * This file avoids it by hand-writing a PRE-v67 `emails` table that genuinely
 * lacks the column — following the v47..v66 convention — and by ASSERTING THE
 ...
 *  7. IT NO-OPS WITHOUT `emails`, mirroring v48/v52..v66, so a minimal
 *     partial-schema fixture does not throw.
 *
 * Follows the v47..v66 convention: real better-sqlite3 driver, in-memory DB via
 * createMigrationHarness, seeded at 66 AND clipped to 67 so ONLY v67 runs.
 */
...
/** The column set `emails` carries BEFORE v67, in order. */
const PRE_V67_EMAIL_COLUMNS = [
  "id",
  "user_id",
  "subject",
  "body_plain",
...
describe("databaseService migration v67 (BACKLOG-2857 — derivation provenance)", () => {
  let harness: MigrationHarness;

  beforeEach(() => {
    harness = createMigrationHarness({ seedV29Schema: false });
    harness.db.exec(PRE_V67_FIXTURE);
  ...
  /** Seed at v66 AND clip the chain at v67 so ONLY v67 runs. */
  async function runV67(): Promise<void> {
    harness.db.prepare("INSERT OR REPLACE INTO schema_version (id, version) VALUES (1, 66)").run();
    const klass = harness.service.constructor as { MIGRATIONS: Array<{ version: number }> };
    const all = klass.MIGRATIONS;
    klass.MIGRATIONS = all.filter((m) => m.version <= 67);
    try {
  ...
  it("advances schema_version to 67", async () => {
    await runV67();
    expect(schemaVersion(harness.db)).toBe(67);
  });
  ...
  it("is idempotent, including the index", async () => {
    ...
    // Re-running from 66 replays v67 against a database that already has both.
    await expect(runV67()).resolves.not.toThrow();
