...
describe("DatabaseService Migration Robustness (TASK-2048)", () => {
  ...
  describe("dry-run mode", () => {
    it("should return migration plan without executing", async () => {
      ...
      expect(plan).toEqual({
        ...
        pendingMigrations: [
          ...
          {
            version: 64,
            description: expect.stringContaining("BACKLOG-2753"),
          },
        ],
        wouldRunCount: 35,
      });
    ...
    it("should report zero pending when all applied", async () => {
      ...
      // Setup: version = 64 (all applied — BACKLOG-2753's v64, the phone
  ...
  describe("happy path", () => {
    it("should run all pending migrations in sequence", async () => {
      ...
      // count once went back to 33. It went to 34 for BACKLOG-2750's v63 and is
      // 35 for BACKLOG-2753's v64, and the enumeration is doing its job in both
      // directions.
    ...
    it("should skip already-applied migrations", async () => {
      ...
      // version = 64, all migrations applied (BACKLOG-2753's v64 — the
      // re-key of every persisted phone lookup key — is the chain head)
      mockStatement.get
        .mockReturnValueOnce({ name: "schema_version" })
        .mockReturnValueOnce({ version: 64 });
