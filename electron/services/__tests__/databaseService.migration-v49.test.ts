...
describe("databaseService migration v49 (BACKLOG-1900 P0.4 — contact-source backfill)", () => {
  ...
  it("advances schema_version to the latest migration (v52 after BACKLOG-2319)", async () => {
    const migrations = harness.service.constructor.MIGRATIONS as Array<{ version: number }>;
    const latest = migrations[migrations.length - 1].version;
    // BACKLOG-2013 added v51 (first_exported_at); BACKLOG-2319 added v52
    // (communications/ignored_communications match_reason) — now the latest.
    expect(latest).toBe(52);

    // runV49 seeds at 48 then runs ALL pending migrations, so v49..v52 run.
    await runV49();
  ...
  describe("data safety", () => {
    ...
    it("is idempotent — re-running the runner does not re-touch reclassified rows or error", async () => {
      ...
      // Second run: version is already at the latest (v52), so the runner selects
  ...
  describe("fresh-install path (real schema.sql, then migrations)", () => {
    it("reaches the latest version (v52) and does not error on an empty install", async () => {
      await harness.cleanup();
      harness = createMigrationHarness({ seedV29Schema: false });
