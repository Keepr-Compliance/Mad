/**
 * BACKLOG-3086 launder fixture — Seam B / P6 — an ambient declaration has no implementation to check.
 *
...
declare function ambientLaunder(s: string): SafeSql;

const handWritten = `SELECT id FROM contacts WHERE display_name = 'x'`;

export const laundered = dbAll(ambientLaunder(handWritten), ["x"]);
