      ...
      <div className="flex-1 min-h-0 overflow-y-auto p-3 sm:p-6">
        {isLoading && items.length === 0 ? (
          ...
          <div className="mx-auto mt-16 max-w-sm text-center">
            ...
            <p className="mt-2 font-medium text-gray-900">Everything is reviewed</p>
            <p className="mt-1 text-sm text-gray-600">
              New communications will appear here when they are found.
            </p>
            {/* Same handler the header's "Back to Transaction" button uses — this
                is the close affordance, not a new navigation path. */}
            <button
              type="button"
              onClick={onClose}
              className="mt-6 inline-flex items-center justify-center rounded-lg bg-gradient-to-r from-amber-500 to-orange-500 px-6 py-3 font-medium text-white shadow-md transition-all hover:from-amber-600 hover:to-orange-600"
              data-testid="needs-review-empty-see-transaction"
            >
              See Transaction
            </button>
          </div>
