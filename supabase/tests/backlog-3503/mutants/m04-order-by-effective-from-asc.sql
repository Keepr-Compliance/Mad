...
AS $fn$ SELECT a.* FROM public.agent_commission_agreements a
   ...
   ORDER BY a.effective_from ASC, a.seq DESC LIMIT 1; $fn$;
DO $m$ BEGIN
  IF position('effective_from ASC' in (SELECT prosrc FROM pg_proc WHERE oid='public.commission_agreement_in_force(uuid,uuid,date)'::regprocedure)) = 0
  THEN RAISE EXCEPTION 'MUTATION NOT APPLIED'; END IF; END $m$;
SELECT 'MUTATION APPLIED: ORDER BY effective_from ASC -- the OLDEST agreement wins';
