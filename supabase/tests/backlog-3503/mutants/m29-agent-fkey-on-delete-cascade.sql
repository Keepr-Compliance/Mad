-- The founder's rule inverted: deleting a user silently takes their split
...
ALTER TABLE public.agent_split_agreements DROP CONSTRAINT agent_split_agreements_agent_fkey;
ALTER TABLE public.agent_split_agreements
  ADD CONSTRAINT agent_split_agreements_agent_fkey
  FOREIGN KEY (agent_user_id) REFERENCES public.users(id) ON DELETE CASCADE;
DO $m$ BEGIN
  IF (SELECT confdeltype FROM pg_constraint WHERE conname = 'agent_split_agreements_agent_fkey') <> 'c'
