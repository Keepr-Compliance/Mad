...
DO $m$ BEGIN
  IF (SELECT relrowsecurity FROM pg_class WHERE oid = 'public.agent_split_agreements'::regclass)
  THEN RAISE EXCEPTION 'MUTATION NOT APPLIED'; END IF; END $m$;
SELECT 'MUTATION APPLIED: ENABLE ROW LEVEL SECURITY missing on agent_split_agreements';
