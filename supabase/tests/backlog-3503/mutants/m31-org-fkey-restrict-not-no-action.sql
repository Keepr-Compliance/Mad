...
-- assertions above it.
ALTER TABLE public.agent_commission_agreements DROP CONSTRAINT agent_commission_agreements_org_fkey;
ALTER TABLE public.agent_commission_agreements
  ADD CONSTRAINT agent_commission_agreements_org_fkey
  FOREIGN KEY (organization_id) REFERENCES public.organizations(id) ON DELETE RESTRICT;
DO $m$ BEGIN
  IF (SELECT confdeltype FROM pg_constraint WHERE conname = 'agent_commission_agreements_org_fkey') <> 'r'
