-- can_write_split_agreements is one of the migration's two SECURITY DEFINER
-- objects -- is_active_split_member is the other -- and this mutant touches
...
ALTER FUNCTION public.can_write_split_agreements(uuid) RESET search_path;
DO $m$ BEGIN
  IF (SELECT proconfig FROM pg_proc WHERE oid = 'public.can_write_split_agreements(uuid)'::regprocedure) IS NOT NULL
