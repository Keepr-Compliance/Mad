...
BEGIN
  SELECT set_at INTO a FROM public.agent_split_agreements
    WHERE effective_from = DATE '2026-06-01' AND agent_pct = 50.00;
  SELECT set_at INTO b FROM public.agent_split_agreements
  ...
  PERFORM pg_temp.check((SELECT seq FROM public.agent_split_agreements WHERE effective_from = DATE '2026-06-01' AND agent_pct = 80.00)
                      > (SELECT seq FROM public.agent_split_agreements WHERE effective_from = DATE '2026-06-01' AND agent_pct = 50.00),
                        'and a LATER seq');
  SELECT agent_pct INTO p FROM public.split_agreement_in_force(
