...
DO $$ BEGIN
  PERFORM pg_temp.check((SELECT count(*) FROM public.agent_commission_agreements) = 4,
                        'agent A sees exactly their own 4 rows');
  PERFORM pg_temp.check(NOT EXISTS (SELECT 1 FROM public.agent_commission_agreements
