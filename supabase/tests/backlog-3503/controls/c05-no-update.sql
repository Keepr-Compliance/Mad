-- C5: a broker cannot UPDATE either table. Asserts the SPECIFIC outcome: with no
...
BEGIN
  SELECT agent_pct INTO before FROM public.agent_commission_agreements
    WHERE agent_user_id = current_setting('t3503.u_agent_a')::uuid AND effective_from = DATE '2026-01-01';
  s := pg_temp.sqlstate_of('UPDATE public.agent_commission_agreements SET agent_pct = 1');
  PERFORM pg_temp.check(s = '42501', format('agreement UPDATE refused with 42501, got %s', s));
  s := pg_temp.sqlstate_of('UPDATE public.organization_franchise_fees SET amount = 1');
  PERFORM pg_temp.check(s = '42501', format('franchise UPDATE refused with 42501, got %s', s));
  SELECT agent_pct INTO after FROM public.agent_commission_agreements
    WHERE agent_user_id = current_setting('t3503.u_agent_a')::uuid AND effective_from = DATE '2026-01-01';
