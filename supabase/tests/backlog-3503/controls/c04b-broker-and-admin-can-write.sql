-- C4b: broker AND admin of the org can insert on both tables. Paired with C4:
...
BEGIN
  ...
  FOREACH who IN ARRAY ARRAY['broker', 'admin'] LOOP
    ...
    s := pg_temp.sqlstate_of(format(
      'INSERT INTO public.agent_commission_agreements (organization_id, agent_user_id, agent_pct, brokerage_pct, office_fee_amount, office_fee_cadence, effective_from) VALUES (%L,%L,65,35,120,%L,DATE ''2026-07-01'')',
    ...
    s := pg_temp.sqlstate_of(format(
      'INSERT INTO public.organization_franchise_fees (organization_id, amount, effective_from) VALUES (%L,2600,DATE ''2026-07-01'')',
      current_setting('t3503.o_a')));
    PERFORM pg_temp.check(s = 'OK', format('franchise INSERT permitted for the org %s, got %s', who, s));
    RESET ROLE;
  END LOOP;
END $$;
RESET ROLE;
