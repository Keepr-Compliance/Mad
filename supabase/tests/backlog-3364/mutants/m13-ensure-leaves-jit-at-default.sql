...
-- ensure does not set jit_provisioning_enabled (column default true)
...
BEGIN
  ...
  VALUES
    ('Personal', 'personal-' || replace(p_user_id::text, '-', ''), 1, 'agent', p_user_id)
...
DO $proof$
BEGIN
  IF NOT (pg_get_functiondef('public._ensure_personal_organization_for(uuid)'::regprocedure) NOT LIKE '%jit_provisioning_enabled%') THEN
    RAISE EXCEPTION 'MUTATION NOT APPLIED';
  END IF;
END
$proof$;
SELECT 'MUTATION APPLIED: ' || ('jit column not set') AS mutation;
