...
-- ensure does not require a license
...
DO $proof$
BEGIN
  IF NOT (pg_get_functiondef('public._ensure_personal_organization_for(uuid)'::regprocedure) NOT LIKE '%no_license%') THEN
    RAISE EXCEPTION 'MUTATION NOT APPLIED';
  END IF;
END
$proof$;
SELECT 'MUTATION APPLIED: ' || ('license check removed') AS mutation;
