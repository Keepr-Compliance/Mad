...
-- ensure deletes a member-less personal organization and creates a new one
...
DO $proof$
BEGIN
  IF NOT (pg_get_functiondef('public._ensure_personal_organization_for(uuid)'::regprocedure) LIKE '%DELETE FROM public.organizations o%') THEN
    RAISE EXCEPTION 'MUTATION NOT APPLIED';
  END IF;
END
$proof$;
SELECT 'MUTATION APPLIED: ' || ('recreate branch present') AS mutation;
