...
-- ensure's membership INSERT has no ON CONFLICT
...
DO $proof$
BEGIN
  IF NOT (pg_get_functiondef('public._ensure_personal_organization_for(uuid)'::regprocedure) NOT LIKE '%ON CONFLICT (organization_id, user_id)%') THEN
    RAISE EXCEPTION 'MUTATION NOT APPLIED';
  END IF;
END
$proof$;
SELECT 'MUTATION APPLIED: ' || ('membership ON CONFLICT removed') AS mutation;
