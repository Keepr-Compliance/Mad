...
-- ensure treats an EXPIRED invite as blocking (expiry test removed)
...
DO $proof$
BEGIN
  IF NOT (pg_get_functiondef('public._ensure_personal_organization_for(uuid)'::regprocedure) NOT LIKE '%invitation_expires_at%') THEN
    RAISE EXCEPTION 'MUTATION NOT APPLIED';
  END IF;
END
$proof$;
SELECT 'MUTATION APPLIED: ' || ('expiry test removed') AS mutation;
