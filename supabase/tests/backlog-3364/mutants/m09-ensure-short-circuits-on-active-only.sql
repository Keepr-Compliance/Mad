...
-- ensure short-circuits only on ACTIVE brokerage memberships
...
DO $proof$
BEGIN
  IF NOT (pg_get_functiondef('public._ensure_personal_organization_for(uuid)'::regprocedure) LIKE '%m.license_status = ''active''%') THEN
    RAISE EXCEPTION 'MUTATION NOT APPLIED';
  END IF;
END
$proof$;
SELECT 'MUTATION APPLIED: ' || ('short-circuit filtered to active') AS mutation;
