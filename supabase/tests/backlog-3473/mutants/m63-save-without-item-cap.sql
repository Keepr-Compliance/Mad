...
-- BACKLOG-3474 A6: the 1..200 item bound removed from the function (portal-only cap)
-- targets: c33
...
DO $proof$
BEGIN
  IF NOT (pg_get_functiondef('public.save_checklist_template(uuid,uuid,text,text,text,jsonb)'::regprocedure) NOT LIKE '%NOT BETWEEN 1 AND 200%') THEN
    RAISE EXCEPTION 'MUTATION NOT APPLIED';
  END IF;
END
$proof$;
SELECT 'MUTATION APPLIED: ' || ('no item cap') AS mutation;
