...
-- BACKLOG-3474 M18: the kept-id array is built from every element, NULL ids included
-- targets: c36
...
DO $proof$
BEGIN
  IF NOT (pg_get_functiondef('public.save_checklist_template(uuid,uuid,text,text,text,jsonb)'::regprocedure) LIKE '%AS e(value)));%') THEN
    RAISE EXCEPTION 'MUTATION NOT APPLIED';
  END IF;
END
$proof$;
SELECT 'MUTATION APPLIED: ' || ('kept ids include NULLs') AS mutation;
