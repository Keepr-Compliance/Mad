...
-- BACKLOG-3474 M16: the item UPDATE drops `i.template_id = v_template_id`
-- targets: c32
...
DO $proof$
BEGIN
  IF NOT (pg_get_functiondef('public.save_checklist_template(uuid,uuid,text,text,text,jsonb)'::regprocedure) NOT LIKE '%AND i.template_id = v_template_id%') THEN
    RAISE EXCEPTION 'MUTATION NOT APPLIED';
  END IF;
END
$proof$;
SELECT 'MUTATION APPLIED: ' || ('item update not scoped to the template') AS mutation;
