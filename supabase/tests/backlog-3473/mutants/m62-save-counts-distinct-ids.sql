...
-- BACKLOG-3474 A5: the id count is DISTINCT, so a repeated id matches the updated-row count
-- targets: c32
...
BEGIN
  ...
  SELECT count(DISTINCT e.value->>'id') INTO v_id_elems
...
DO $proof$
BEGIN
  IF NOT (pg_get_functiondef('public.save_checklist_template(uuid,uuid,text,text,text,jsonb)'::regprocedure) LIKE '%count(DISTINCT%') THEN
    RAISE EXCEPTION 'MUTATION NOT APPLIED';
  END IF;
END
$proof$;
SELECT 'MUTATION APPLIED: ' || ('distinct id count') AS mutation;
