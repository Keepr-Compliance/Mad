...
-- broker_get_org_features restored to its pre-migration body (lib/rpc-before.sql): no min-tier guard
-- targets: c15 c16
...
DO $proof$
BEGIN
  IF NOT (pg_get_functiondef('public.broker_get_org_features(uuid)'::regprocedure) NOT LIKE '%_override_above_tier%') THEN
    RAISE EXCEPTION 'MUTATION NOT APPLIED';
  END IF;
END
$proof$;
SELECT 'MUTATION APPLIED: ' || ('broker_get_org_features has no guard call') AS mutation;
