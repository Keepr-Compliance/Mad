...
-- BACKLOG-3474 A2: the template UPDATE is skipped when name and description are unchanged
-- targets: c30
...
DO $proof$
BEGIN
  IF NOT (pg_get_functiondef('public.save_checklist_template(uuid,uuid,text,text,text,jsonb)'::regprocedure) LIKE '%IS NOT DISTINCT FROM NULLIF(btrim(p_description)%') THEN
    RAISE EXCEPTION 'MUTATION NOT APPLIED';
  END IF;
END
$proof$;
SELECT 'MUTATION APPLIED: ' || ('update skipped when header unchanged') AS mutation;
