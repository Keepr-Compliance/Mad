...
-- can_edit_checklist_templates without the owner clause (the item not implemented)
-- targets: c41
...
DO $proof$
BEGIN
  IF NOT (pg_get_functiondef('public.can_edit_checklist_templates(uuid)'::regprocedure) NOT LIKE '%personal_owner_user_id%') THEN
    RAISE EXCEPTION 'MUTATION NOT APPLIED';
  END IF;
END
$proof$;
SELECT 'MUTATION APPLIED: ' || ('owner clause absent') AS mutation;
