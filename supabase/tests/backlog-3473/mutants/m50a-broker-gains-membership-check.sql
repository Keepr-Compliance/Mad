...
-- broker_get_org_features refuses non-members, like get_org_features
-- targets: c16
...
DO $proof$
BEGIN
  IF NOT (pg_get_functiondef('public.broker_get_org_features(uuid)'::regprocedure) LIKE '%not_authorized%') THEN
    RAISE EXCEPTION 'MUTATION NOT APPLIED';
  END IF;
END
$proof$;
SELECT 'MUTATION APPLIED: ' || ('broker checks membership') AS mutation;
