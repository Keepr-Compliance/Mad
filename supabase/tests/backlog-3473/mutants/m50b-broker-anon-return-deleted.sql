...
-- broker_get_org_features has no early return for an unauthenticated caller
-- targets: c16
...
DO $proof$
BEGIN
  IF NOT (pg_get_functiondef('public.broker_get_org_features(uuid)'::regprocedure) NOT LIKE '%not_authenticated%') THEN
    RAISE EXCEPTION 'MUTATION NOT APPLIED';
  END IF;
END
$proof$;
SELECT 'MUTATION APPLIED: ' || ('no anon early return') AS mutation;
