...
-- BACKLOG-3474 A7: the file replayed on a fresh function with `anon` dropped from the REVOKE
-- targets: c34
...
REVOKE EXECUTE ON FUNCTION public.save_checklist_template(uuid, uuid, text, text, text, jsonb) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.save_checklist_template(uuid, uuid, text, text, text, jsonb) TO authenticated;


DO $proof$
BEGIN
  IF NOT (has_function_privilege('anon', 'public.save_checklist_template(uuid,uuid,text,text,text,jsonb)', 'EXECUTE')) THEN
    RAISE EXCEPTION 'MUTATION NOT APPLIED';
  END IF;
END
$proof$;
SELECT 'MUTATION APPLIED: ' || ('anon can execute') AS mutation;
