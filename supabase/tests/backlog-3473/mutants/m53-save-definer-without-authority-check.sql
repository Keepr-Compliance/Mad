...
-- BACKLOG-3474 M15b: no authority check AND SECURITY DEFINER (RLS no longer applies to the writes)
-- targets: c35
...
DO $proof$
BEGIN
  IF NOT (pg_get_functiondef('public.save_checklist_template(uuid,uuid,text,text,text,jsonb)'::regprocedure) NOT LIKE '%can_edit_checklist_templates%' AND (SELECT prosecdef FROM pg_proc WHERE oid = 'public.save_checklist_template(uuid,uuid,text,text,text,jsonb)'::regprocedure)) THEN
    RAISE EXCEPTION 'MUTATION NOT APPLIED';
  END IF;
END
$proof$;
SELECT 'MUTATION APPLIED: ' || ('no authority check, security definer') AS mutation;
