...
-- BACKLOG-3474 A3: the function hands back the caller's token instead of the new updated_at
-- targets: c31
...
BEGIN
  ...
  RETURN QUERY SELECT v_template_id, coalesce(p_expected_updated_at, to_json(v_updated_at) #>> '{}');
END
$$;

DO $proof$
BEGIN
  IF NOT (pg_get_functiondef('public.save_checklist_template(uuid,uuid,text,text,text,jsonb)'::regprocedure) LIKE '%coalesce(p_expected_updated_at%') THEN
    RAISE EXCEPTION 'MUTATION NOT APPLIED';
  END IF;
END
$proof$;
SELECT 'MUTATION APPLIED: ' || ('returns the caller token') AS mutation;
