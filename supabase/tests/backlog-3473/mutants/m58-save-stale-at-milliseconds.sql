...
-- BACKLOG-3474 M17: the stale check compares at millisecond precision
-- targets: c29
...
DO $proof$
BEGIN
  IF NOT (pg_get_functiondef('public.save_checklist_template(uuid,uuid,text,text,text,jsonb)'::regprocedure) LIKE '%date_trunc(''milliseconds''%') THEN
    RAISE EXCEPTION 'MUTATION NOT APPLIED';
  END IF;
END
$proof$;
SELECT 'MUTATION APPLIED: ' || ('stale check at ms') AS mutation;
