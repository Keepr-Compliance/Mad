...
-- submission_checklists INSERT without the submitter term
-- targets: c09
...
DO $proof$
BEGIN
  IF NOT ((SELECT with_check FROM pg_policies WHERE schemaname = 'public' AND policyname = 'submission_checklists_insert') NOT LIKE '%submitted_by%') THEN
    RAISE EXCEPTION 'MUTATION NOT APPLIED';
  END IF;
END
$proof$;
SELECT 'MUTATION APPLIED: ' || ('header submitter term dropped') AS mutation;
