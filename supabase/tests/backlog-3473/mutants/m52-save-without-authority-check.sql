...
-- BACKLOG-3474 A9: save_checklist_template without its up-front can_edit_checklist_templates check
-- targets: c35
...
DO $proof$
BEGIN
  IF NOT (pg_get_functiondef('public.save_checklist_template(uuid,uuid,text,text,text,jsonb)'::regprocedure) NOT LIKE '%can_edit_checklist_templates%') THEN
    RAISE EXCEPTION 'MUTATION NOT APPLIED';
  END IF;
END
$proof$;
SELECT 'MUTATION APPLIED: ' || ('no authority check') AS mutation;
