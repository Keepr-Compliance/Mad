...
-- BACKLOG-3474 M15: save_checklist_template made SECURITY DEFINER (authority check kept)
-- targets: c34
...
DO $proof$
BEGIN
  IF NOT ((SELECT prosecdef FROM pg_proc WHERE oid = 'public.save_checklist_template(uuid,uuid,text,text,text,jsonb)'::regprocedure)) THEN
    RAISE EXCEPTION 'MUTATION NOT APPLIED';
  END IF;
END
$proof$;
SELECT 'MUTATION APPLIED: ' || ('security definer') AS mutation;
