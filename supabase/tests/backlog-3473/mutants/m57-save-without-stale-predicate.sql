...
-- BACKLOG-3474 M17: the template UPDATE no longer compares updated_at with the caller's token
-- targets: c29 c30
...
DO $proof$
BEGIN
  IF NOT (pg_get_functiondef('public.save_checklist_template(uuid,uuid,text,text,text,jsonb)'::regprocedure) NOT LIKE '%p_expected_updated_at::timestamptz%') THEN
    RAISE EXCEPTION 'MUTATION NOT APPLIED';
  END IF;
END
$proof$;
SELECT 'MUTATION APPLIED: ' || ('no stale predicate') AS mutation;
