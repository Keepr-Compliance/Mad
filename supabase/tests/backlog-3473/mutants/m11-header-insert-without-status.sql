...
-- submission_checklists INSERT without the 'uploading' term
-- targets: c09
...
DO $proof$
BEGIN
  IF NOT ((SELECT with_check FROM pg_policies WHERE schemaname = 'public' AND policyname = 'submission_checklists_insert') NOT LIKE '%uploading%') THEN
    RAISE EXCEPTION 'MUTATION NOT APPLIED';
  END IF;
END
$proof$;
SELECT 'MUTATION APPLIED: ' || ('header status term dropped') AS mutation;
